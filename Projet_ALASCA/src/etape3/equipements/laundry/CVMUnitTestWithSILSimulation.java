...
// -----------------------------------------------------------------------------
/**
 * The class <code>CVMUnitTestWithSILSimulation</code> performs unit tests
 * for the laundry machine component with SIL simulation.
 *
 * <p>Created on : 2026-02-06</p>
 */
public class CVMUnitTestWithSILSimulation extends AbstractCVM {
	// -------------------------------------------------------------------------
	...
	public static Duration SIMULATION_DURATION =
		new Duration(12.0, SIMULATION_TIME_UNIT);
	...
	public CVMUnitTestWithSILSimulation() throws Exception {
	...
	public static void main(String[] args) {
		try {
			...
			CVMUnitTestWithSILSimulation cvm =
				new CVMUnitTestWithSILSimulation();
	...
	public static TestScenario unitTestScenario() throws VerboseException {
		...
		Instant setDelicateModeInstant = startInstant.plusSeconds(1200);
		Instant setColorModeInstant = startInstant.plusSeconds(1500);
		Instant setIntensiveModeInstant = startInstant.plusSeconds(1800);
		Instant startWashInstant = startInstant.plusSeconds(2700);
		Instant cancelWashInstant = startInstant.plusSeconds(8700);
		...
		return new TestScenario(
			...
			new TestStepI[] {
				...
				new TestStep(
					CLOCK_URI,
					LaundryTesterCyPhy.REFLECTION_INBOUND_PORT_URI,
					setIntensiveModeInstant,
					owner -> {
						try {
							((LaundryTesterCyPhy)owner).getLuOP().setIntensiveMode();
						} catch (Exception e) {
							throw new BCMRuntimeException(e);
						}
					}),
	...
	throws VerboseException {
		...
		Instant switchOnInstant = startInstant.plusSeconds(300);
		Instant setIntensiveModeInstant = startInstant.plusSeconds(900);
		Instant startWashInstant = startInstant.plusSeconds(1200);
		Instant cancelWashInstant = startInstant.plusSeconds(18000);
		Instant switchOffInstant = startInstant.plusSeconds(d - 300);
